...
f=figure('Name', '','Position', [scrnsizes(1,3)/4 scrnsizes(1,4)/7 750 550],...
        'menubar','none','numbertitle','off', 'Color','white');

%% UICONTROLS
    version_string = sprintf('BrainWave (version: %.2f, %s)', BW_VERSION, RELEASE_DATE);
    uicontrol('style','text','units','normalized','position',[0.25 0.9 0.45 0.08],...
        'fontweight','b','foregroundcolor',[0.93,0.5,0.15],'fontsize',14,'backgroundcolor','white','string',version_string);
    
    uicontrol('style','text','units','normalized','position',[0.03 0.87 0.18 0.03],...
         'fontweight','b','fontsize',11,'foregroundcolor','blue','backgroundcolor','white','string','About Brainwave');
    uicontrol('style','text','units','normalized','position',[0.05 0.58 0.85 0.28],...
        'fontsize',10,'fontname','lucinda','HorizontalAlignment','left', 'backgroundcolor','white','string',ABOUT_STR);
    annotation('rectangle',[0.02 0.56 0.95 0.325],'EdgeColor','blue');
    
    
    uicontrol('style','text','units','normalized','position',[0.03 0.51 0.2 0.03],...
        'fontweight','b','fontsize',11,'backgroundcolor','white','foregroundcolor','blue','string','Acknowledgements');
    uicontrol('style','text','units','normalized','position',[0.05 0.235 0.85 0.27],...
        'fontsize',10,'fontname','lucinda','HorizontalAlignment','left','backgroundcolor','white','string',CONTRIB_STR);
    annotation('rectangle',[0.02 0.225 0.95 0.3],'EdgeColor','blue');
    ...
    CONTACT_STR = {'For information or program updates contact: http://cheynelab.utoronto.ca'};
    uicontrol('style','text','units','normalized','position',[0.03 0.04 0.55 0.08],...
        'fontsize',10,'fontname','lucinda','HorizontalAlignment','left','backgroundcolor','white','string',CONTACT_STR);
    annotation('rectangle',[0.02 0.04 0.6 0.1],'EdgeColor','blue');
