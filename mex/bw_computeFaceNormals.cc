...
void bw_computeFaceNormals(double *surface_points_orient,double *surface_points, double *surface_faces,int numberofpoint, int numberofface);


void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]){


	double *surface_points_orient,*surface_points;
	...
	if ( nlhs != n_outputs | nrhs != n_inputs)
	{
		...
		mexPrintf("   input is numVertices x 3 array of vertex locations and numFaces x 3 list of face indices (0 to numFaces-1) \n");
		mexPrintf("   returns normals as a numVertices x 3 array (normal at each vertex which is average of surrounding faces)\n");
		return;
	}
    ...
    surface_points = mxGetPr(prhs[0]);
    surface_faces = mxGetPr(prhs[1]);

    plhs[0] = mxCreateDoubleMatrix(3,numberofpoint,mxREAL);

    surface_points_orient = mxGetPr(plhs[0]);

	bw_computeFaceNormals(surface_points_orient, surface_points, surface_faces, numberofpoint, numberofface);
}
...
void bw_computeFaceNormals( double *surface_points_orient,double *surface_points, double *surface_faces,int numberofpoint, int numberofface){
    ...
    for (j=0;j<numberofface;j++)
    {
        ...
        v1[0] = surface_points[(int)(v1Index*3)];
        v1[1] = surface_points[(int)(v1Index*3+1)];
        v1[2] = surface_points[(int)(v1Index*3+2)];
        
        v2[0] = surface_points[(int)(v2Index*3)];
        v2[1] = surface_points[(int)(v2Index*3+1)];
        v2[2] = surface_points[(int)(v2Index*3+2)];
        
        v3[0] = surface_points[(int)(v3Index*3)];
        v3[1] = surface_points[(int)(v3Index*3+1)];
        v3[2] = surface_points[(int)(v3Index*3+2)];
    ...
    // loop through all vertices and get mean of the normals in each buffer
	 ...
	 for (i=0; i<numberofpoint;i++)
	 {
		 ...
		 surface_points_orient[3*i] = meanvertex_norm[0];
		 surface_points_orient[3*i+1] = meanvertex_norm[1];
		 surface_points_orient[3*i+2] = meanvertex_norm[2];
	 }
