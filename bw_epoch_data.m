...
%
%   Version 5.3  - Jan, 2026            -   renamed bw_epoch_data - does
%                                           epoching only
...
fh=figure('Name', 'Epoch MEG Data', 'Position', [scrnsizes(1,3)/6 scrnsizes(1,4)/2 1300 850],...
...
uicontrol('style','text','units','normalized','position',[0.03 0.95 0.15 0.04],...
        'String','Data Parameters','ForegroundColor','blue','FontSize', 10,...
...
saveDsName_edit=uicontrol('style','edit','units','normalized','position',[0.1 0.835 0.28 0.025],...
    'String',saveDsName,'HorizontalAlignment','left','fontsize',9,'fontweight','bold','BackgroundColor','white');

uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.3 0.80 0.08 0.025],...
...
Fid_text = uicontrol('style','text','units','normalized','position',[0.02 0.47 0.3 0.04],...
    ...
    'BackgroundColor','white');
nasion_txt = uicontrol('style','text','units','normalized','position',[0.02 0.450 0.35 0.04],...
    'String','Nasion:','HorizontalAlignment','left',...%'fontsize',textFontSize,...
    'BackgroundColor','white');
left_ear_txt = uicontrol('style','text','units','normalized','position',[0.02 0.425 0.35 0.04],...
    'String','Left Ear:','HorizontalAlignment','left',...%'fontsize',textFontSize,...
    'BackgroundColor','white');
right_ear_txt= uicontrol('style','text','units','normalized','position',[0.02 0.400 0.35 0.04],...
    'String','Right Ear:','HorizontalAlignment','left',...%'fontsize',textFontSize,...
    'BackgroundColor','white');

%

createButton =  uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.02 0.03 0.1 0.05],...
        'enable','off','string','Create Datasets','Foregroundcolor',orange,'backgroundcolor','white','callback',@create_ds_callback);

addToBatchButton =  uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.14 0.03 0.07 0.05],...
...
function loadDsFile(datafile)
    ...
    % flag bad lowpass setting in file
    if header.lowPass >= header.sampleRate / 2
        header.lowPass = round(header.sampleRate / 2);
    end
    ...
    if hasCHL
        ...
        useMeanHeadPosition = 1;
        set(use_mean_position_check, 'value', useMeanHeadPosition, 'enable','on');
    else
        set(head_motion_check, 'enable','off');
        set(motionThreshold_edit, 'enable','off');
        set(headMotionPlot_button, 'enable','off');
        useMeanHeadPosition = 0;
        set(use_mean_position_check, 'value', useMeanHeadPosition, 'enable','off');
    end           
...
uicontrol('style','text','units','normalized','position',[0.45 0.95 0.13 0.04],...
        'String','Epoch Selection','fontsize',9,'ForegroundColor','blue',...
        'HorizontalAlignment','center','BackGroundColor', 'white','fontweight','b');
...
all_data=uicontrol('style','radio','units','normalized','position',[0.46 0.9 0.15 0.04],...
    'string','Single Epoch','fontsize',9,'backgroundcolor','white','value',~epochFlag,...
...
uicontrol('style','text','units','normalized','position',[0.57 0.89 0.08 0.04],...
    'string','Start (s):','fontsize',9,'backgroundcolor','white','horizontalalignment','left');
single_epoch_start=uicontrol('style','edit','units','normalized','position',[0.61 0.9 0.055 0.04],...
    'string',singleEpochStart,'FontSize', 10, 'BackGroundColor','white','callback',@single_start_callback);

uicontrol('style','text','units','normalized','position',[0.7 0.89 0.08 0.04],...
    'string','End (s):','fontsize',9,'backgroundcolor','white','horizontalalignment','left');
single_epoch_end=uicontrol('style','edit','units','normalized','position',[0.74 0.9 0.055 0.04],...
    'string',singleEpochEnd,'FontSize', 10, 'BackGroundColor','white','callback',@single_end_callback);
...
epoch_data=uicontrol('style','radio','units','normalized','position',[0.46 0.84 0.15 0.04],...
    'string','Multiple Epochs','fontsize',9,'backgroundcolor','white','value',epochFlag,...
    ...
    function updateEpochFields
        ...
        if epochFlag
            ...
            set(load_text,'enable','on')

            set(save_average_check,'enable','on');
            set(plot_average_check,'enable','on');
            ...
            set(load_text,'enable','off')
            set(save_average_check,'enable','off');
            set(plot_average_check,'enable','off');
...
latency_file_label = uicontrol('style','text','units','normalized',...
    'position',[0.53 0.77 0.4 0.04],'String','Event: none','fontsize',9,'ForegroundColor','blue',...
        'HorizontalAlignment','left','BackGroundColor', 'white');

lat_text=uicontrol('style','text','units','normalized','Position',...
    [0.53 0.75 0.15 0.04],'String','Latencies','fontsize',9,'HorizontalAlignment','Left',...
    'BackgroundColor','white');

lat_box=uicontrol('Style','Listbox','fontsize',9,'Units','Normalized','Position',...
    [0.53 0.63 0.12 0.14],'String',latencyList,'HorizontalAlignment','Center',...
    'BackgroundColor','white','min',1,'max',10000,'callback',@lat_box_callback);

uicontrol('style','text','units','normalized','Position',...
    [0.53 0.605 0.15 0.02],'String','( ** = excluded epochs )','fontsize',9,'HorizontalAlignment','Left',...
    'BackgroundColor','white');

%epoch time windows
tw_title=uicontrol('style','text','units','normalized','position',...
    [0.72 0.75 0.1 0.04],'String','Epoch Window:','fontsize',9,'HorizontalAlignment','left',...
    'BackgroundColor','white');
...
tw_start=uicontrol('style','edit','units','normalized','position',[0.88 0.73 0.055 0.035],...
    'string',twStart,'fontsize',9, 'BackGroundColor','white','value',twStart,...
...
tw_end=uicontrol('style','edit','units','normalized','position',[0.88 0.69 0.055 0.035],...
    'string',twEnd,'fontsize',9, 'BackGroundColor','white','value',twEnd,...
...
tw_ol=uicontrol('style','edit','units','normalized','position',[0.88 0.65 0.058 0.035],'enable','off',...
    'string',minSep,'fontsize',9, 'BackGroundColor','white','callback',@tw_ol_callback);
...
correct_txt=uicontrol('style','text','units','normalized','position',[0.72 0.62 0.18 0.02],...
    'string','Latency Correction (s):','backgroundcolor','white','fontsize',9,'horizontalalignment','left');
latency_correct_edit=uicontrol('style','edit','units','normalized','position',[0.88 0.61 0.058 0.035],...
    'string',latencyCorrection,'fontsize',9, 'BackGroundColor','white','callback',@latency_correction_callback);
...
tw_ol_check=uicontrol('style','checkbox','units','normalized','position',[0.72 0.65 0.14 0.035],...
    'string','Min. Separation (s):','backgroundcolor','white','value',useMinSep,'fontsize',9,'callback',@tw_ol_check_callback);

delete_button=uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.43 0.63 0.08 0.04],...
    'string','Delete Event','Foregroundcolor','blue','backgroundcolor','white','callback',@delete_callback);
    function delete_callback(~,~)
        idx = get(lat_box,'value');
        if isempty(idx) || length(idx) > size(latencyList,1)
            return;
        end
        s = sprintf('Delete %d events?', size(currentTrialNo,2));
        response = questdlg(s,'BrainWave','Yes','No','Yes');
        if strcmp(response,'Yes')           
            latencyList(idx) = []; 
            currentTrialNo = idx(end)-1;  
            if currentTrialNo < 1 
                currentTrialNo = 1;
            end
            if currentTrialNo(end) > size(latencyList,1)
                currentTrialNo = size(latencyList,1);
            end
...
load_lat=uicontrol('style','pushbutton','units','normalized','position',[0.55 0.84 0.1 0.04],...
    'string','Load from Marker File','Foregroundcolor','blue','backgroundcolor','white','callback',@load_lat_callback);
...
load_text =uicontrol('style','pushbutton','units','normalized','position',[0.67 0.84 0.1 0.04],...
        'string','Load from Text File','Foregroundcolor','blue','backgroundcolor','white','callback',@load_text_callback);
    
    function load_text_callback(~,~)
        ...
        [name,path,~] = uigetfile('*.*','Select latency file:');
        if isequal(name,0)
            return;
        end
        textFileName = fullfile(path,name);                 

        fid = fopen(textFileName,'r');
        instr = textscan(fid,'%s','delimiter','\n');
        times = instr{1};
        fclose(fid);

        newList = str2double(times);
        ...
        [~,name,ext] = fileparts(textFileName);
        eventFile = [name ext];  

        eventName = name;

        fprintf('Read %d events from %s..\n', size(newList,1), textFileName);
...
uicontrol('style','text','units','normalized','position',[0.45 0.55 0.12 0.04],...
        'String','Pre-processing','fontsize',9,'ForegroundColor','blue',...
        'HorizontalAlignment','center','BackGroundColor', 'white','fontweight','b');

filter_data_check = uicontrol('style','checkbox','units','normalized','position',[0.44 0.51 0.1 0.05],...
    'string','Filter data','value', filterData,'fontsize',9,'backgroundcolor','white','callback',@filter_data_callback);
...
filt_hi_pass=uicontrol('style','edit','units','normalized','position',[0.575 0.515 0.04 0.04],'enable','off',...
    'fontsize',9, 'BackGroundColor','white','string',bandpass(1),...
...
filt_low_pass=uicontrol('style','edit','units','normalized','position',[0.685 0.515 0.04 0.04],'enable','off',...
    'fontsize',9, 'BackGroundColor','white','string',bandpass(2),...
...
extra_samples_check = uicontrol('style','checkbox','units','normalized','position',[0.73 0.51 0.15 0.05],'enable','off',...
    'string','Expand filter window','value', useExtraSamples,'backgroundcolor','white','callback',@extra_samples_callback); %,'fontsize',9
...
lineFilter_data_check = uicontrol('style','checkbox','units','normalized','position',[0.44 0.465 0.1 0.05],...
    'string','Filter powerline','value', lineFilterData,'fontsize',9,'backgroundcolor','white','callback',@lineFilter_data_callback);
...
radio_60Hz = uicontrol('style','radio','units','normalized','position',[0.55 0.465 0.06 0.05],'enable','off',...
    'string','60 Hz','value', val,'backgroundcolor','white','callback',@radio60Hz_callback); %,'fontsize',9
...
radio_50Hz = uicontrol('style','radio','units','normalized','position',[0.6 0.465 0.06 0.05],'enable','off',...
    'string','50 Hz','value', val,'backgroundcolor','white','callback',@radio50Hz_callback); %,'fontsize',9
...
uicontrol('style','text','units','normalized','position',[0.44 0.415 0.1 0.035],...
    'string','Sample Rate:','backgroundcolor','white','fontsize',9,'horizontalalignment','left');

sample_popup = uicontrol('style','popup','units','normalized',...
    'position',[0.53 0.405 0.12 0.05],'String', rates, 'Backgroundcolor','white','fontsize',9,...
...
use_mean_position_check = uicontrol('style','checkbox','units','normalized','position',[0.68 0.425 0.15 0.04],...
    'string','Use mean head position','value',useMeanHeadPosition,'enable','off',...
    'backgroundcolor','white','callback',@use_mean_callback);%'fontsize',9
...
updateHeadPos_Button = uicontrol('style','pushbutton','units','normalized','position',[0.8 0.43 0.05 0.03],...
...
save_average_check = uicontrol('style','checkbox','units','normalized','position',[0.23 0.04 0.1 0.03],...
    'string','Save Average','value',save_average,'fontsize',9,'backgroundcolor','white','callback',@save_average_callback);

    function save_average_callback(src,~)
        save_average=get(src,'value');
    end

plot_average_check = uicontrol('style','checkbox','units','normalized','position',[0.23 0.01 0.1 0.03],...
    'string','Open in DataPlot','value',plotAverage,'fontsize',9,'backgroundcolor','white','callback',@plot_average_callback);

    function plot_average_callback(src,~)
        plotAverage = get(src,'value');
    end


deidentify_check = uicontrol('style','checkbox','units','normalized','position',[0.32 0.04 0.1 0.03],...
    'string','De-identify Data','value',deidentifyData,'fontsize',9,'backgroundcolor','white','callback',@deidentify_callback);
...
uicontrol('style','text','units','normalized','position',[0.45 0.36 0.08 0.04],...
        'String','Preview','fontsize',10,'ForegroundColor','blue',...
        'HorizontalAlignment','center','BackGroundColor', 'white','fontweight','b');
   
preview_text=uicontrol('style','text','units','normalized','fontsize',9,'position',...
    [0.6 0.35 0.35 0.03],'string','','Foregroundcolor','black','horizontalAlignment','left','backgroundcolor','white');

uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.43 0.05 0.08 0.035],...
...
trialInc =uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.88 0.03 0.08 0.03],...
    'string','Trial >','Foregroundcolor','blue','backgroundcolor','white','callback',@trial_inc_callback);
trialDec =uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.58 0.03 0.08 0.03],...
...
uicontrol('style','checkbox','units','normalized','position',[0.43 0.35 0.12 0.02],...
    'string','Remove offset','value', removeOffset,'fontsize',9,'backgroundcolor','white','callback',@remove_offset_callback);
...
uicontrol('style','checkbox','units','normalized','position',[0.43 0.325 0.12 0.02],...
    'string','Show Bad Channels','value', plotBad,'fontsize',9,'backgroundcolor','white','callback',@plot_bad_callback);
    function plot_bad_callback(src,~)
        plotBad=get(src,'value');
        drawTrial;
    end

uicontrol('style','text','units','normalized','position',[0.43 0.28 0.1 0.04],...
        'String','MEG Channel','fontsize',9,'ForegroundColor','black','HorizontalAlignment','left','BackGroundColor', 'white');

channel_box=uicontrol('Style','Listbox','fontsize',9,'Units','Normalized','Position',...
...
uicontrol('style','text','units','normalized','position',[0.03 0.36 0.15 0.04],...
        'String','Epoch Rejection','fontsize',9,'ForegroundColor','blue',...
        'HorizontalAlignment','center','BackGroundColor', 'white','fontweight','b');
      
uicontrol('style','text','units','normalized','position',[0.04 0.335 0.27 0.04],...
  'String','Exclude epoch if...','HorizontalAlignment','left',...%'fontsize',textFontSize,...
  'BackgroundColor','white');
    
peakToPeak_check = uicontrol('style','checkbox','units','normalized','position',[0.06 0.32 0.27 0.04],...
    'string','Peak-peak amplitude exceeds','value',exclude_BadTrials,'backgroundcolor','white','callback',@peakToPeak_callback);%,'fontsize',9
...
peakThreshold_edit=uicontrol('style','edit','units','normalized','position',[0.2 0.325 0.04 0.035],...
    'fontsize',9, 'BackGroundColor','white','string',peakThreshold * 1e12,'callback',@peakThreshold_callback);
...
uicontrol('style','text','units','normalized','position',[0.25 0.31 0.06 0.04],...
        'String','pT','ForegroundColor','black','HorizontalAlignment','left','BackGroundColor', 'white');
 
reset_check = uicontrol('style','checkbox','units','normalized','position',[0.06 0.27 0.27 0.04],...
    'string','Amplitude step (reset) exceeds','value',exclude_Resets,'backgroundcolor','white','callback',@reset_callback);%,'fontsize',9
...
resetThreshold_edit=uicontrol('style','edit','units','normalized','position',[0.2 0.275 0.04 0.035],...
    'fontsize',9, 'BackGroundColor','white','string',resetThreshold *1e12,'callback',@resetThreshold_callback);
...
head_motion_check = uicontrol('style','checkbox','units','normalized','position',[0.06 0.22 0.27 0.04],...
    'string','Mean sensor motion exceeds','value',exclude_HeadMotion,'backgroundcolor','white','callback',@head_motion_callback);%,'fontsize',9
...
motionThreshold_edit=uicontrol('style','edit','units','normalized','position',[0.2 0.225 0.04 0.035],...
    'fontsize',9, 'BackGroundColor','white','string',motionThreshold,'callback',@motionThreshold_callback);
...
uicontrol('style','text','units','normalized','position',[0.25 0.21 0.07 0.04],...
        'String','cm','ForegroundColor','black','HorizontalAlignment','left','BackGroundColor', 'white');

    
bad_channel_check = uicontrol('style','checkbox','units','normalized','position',[0.02 0.17 0.27 0.04],...
    'string','Exclude Channel if number of trials rejected exceeds','value',exclude_BadChannels,'backgroundcolor','white','callback',@bad_channel_callback);%,'fontsize',9
...
badChannel_edit=uicontrol('style','edit','units','normalized','position',[0.23 0.175 0.04 0.035],...
    'fontsize',9, 'BackGroundColor','white','string',s,'callback',@badChannelThreshold_callback);
...
uicontrol('style','text','units','normalized','position',[0.28 0.16 0.07 0.04],...
        'String','percent','ForegroundColor','black','HorizontalAlignment','left','BackGroundColor', 'white');
...
selected_only_check = uicontrol('style','checkbox','units','normalized','position',[0.2 0.12 0.15 0.04],...
    ...
    'fontsize',9,'backgroundcolor','white','callback',@selected_only_callback);

    function selected_only_callback(src,~)
        useSelectedChannels=get(src,'value');        
    end

uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.03 0.12 0.08 0.035],...
...
uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.13 0.12 0.05 0.035],...
...
headMotionPlot_button=uicontrol('style','pushbutton','units','normalized','fontsize',9,'position',[0.28 0.225 0.08 0.03],...
...
function trial_inc_callback(~,~)
    ...
    currentTrialNo = currentTrialNo(end);
...
function drawTrial      
    ...
    % for now just draw first selected trial....
    ...
    if ~epochFlag
        ...
        latency = latencyList(trialToDraw) + latencyCorrection;
    ...
    for k=1:numChannelsToPlot
        ...
        pcol = 'blue';
        if (epochFlag && validFlags(trialToDraw) == 0) || validChan(channelNo) == 0
            pcol = 'red';             
        end           
...
function load_params_callback(~,~)
   ...
   [name,path,~] = uigetfile('*.mat','Select parameter file:');
    ...
    set(plot_average_check,'value', plotAverage);
...
%         end
    ...
    if errorFlag == 0
            ...
            for k=1:numel(markerData)
                ...
                for trial=1:numel(validLat)  % for this trial
                    for j=1:numel(markerLatencies)      % check all event marker times
                        ...
                        % fix in 5.21 -- was excluding markers if exactly
                        % at window boundaries - also flag no markers
                        %
        ...
        % ** new in version 5.0 if there is a head model file in raw dataset save a copy 

        headModel = strcat(loadfull,filesep,'singleSphere.hdm');
        if exist(headModel,'file') 
            headModelFile = sprintf('%s%s%s',saveName,filesep,'singleSphere.hdm');
            fprintf('Copying head model %s \n', headModelFile);
            cmd = sprintf('cp %s %s',headModel, headModelFile);
            system(cmd);
        end
          
        headModel = strcat(loadfull,filesep,'multipleSphere.hdm');
        if exist(headModel,'file') 
            headModelFile = sprintf('%s%s%s',saveName,filesep,'multipleSphere.hdm');
            fprintf('Copying head model file %s \n', headModel);
            cmd = sprintf('cp %s %s',headModel, headModelFile);
            system(cmd);
        end

        % in version 5.1 - save the header information in a .mat file
        % get header of new dataset
        newHeader = bw_CTFGetHeader(saveName);
        [~,rootName,~] = fileparts(saveName);
        headerName = sprintf('%s%s%s.mat',saveName,filesep, rootName);
        fprintf('Saving header information in %s\n',headerName)
        save(headerName,'-struct','newHeader');
