...
LATENCY_SLIDER = uicontrol('style','slider','units', 'normalized',...
    'position',[0.12 0.035 0.3 0.03],'min',1,'max',2,'Value',1,...
...
LATENCY_TEXT = uicontrol('style','text','units', 'normalized',...
    'position',[0.14 0.065 0.8 0.04 ],'FontSize',...
    11, 'HorizontalAlignment','left','BackGroundColor', 'white', 'string',latStr);

LOOP_BUTTON = uicontrol('style','pushbutton','units', 'normalized',...
    'position',[0.44 0.03 0.06 0.04],'String','LOOP','FontWeight','bold','FontSize',10,...
...
THRESH_EDIT = uicontrol('style','edit','units', 'normalized',...
    'position',[0.08 0.48 0.06 0.05],'String',threshStr,...
	'BackGroundColor','white','FontSize', 11,'callback',@threshEdit_Callback);
    
THRESH_TITLE=uicontrol('style','text','units', 'normalized',...
    'position',[0.08 0.53 0.1 0.03],'String','Threshold:',...
...
THRESH_SLIDER = uicontrol('style','slider','units', 'normalized',...
    'position',[0.045 0.37 0.025 0.28],'min',0,'max',1.0,...
...
ZERO_LABEL=uicontrol('style','text','units', 'normalized',...
    'position',[0.04 0.315 0.05 0.03],'String', '0.0 %','FontSize',...
    11,'BackgroundColor','white');

MAX_PERCENT_LABEL=uicontrol('style','text','units', 'normalized',...
    'position',[0.04 0.67 0.05 0.03],'String', '100 %','FontSize',...
    11,'BackgroundColor','white');
...
MAX_EDIT = uicontrol('style','edit','units', 'normalized',...
    ...
    'white','TooltipString','Enter Threshold','FontSize', 11,...
...
PLOT_NEG_RADIO = uicontrol('style','radio','units', 'normalized',...
    'position',[0.67 0.48 0.14 0.05],'String','Plot negative',...
    'BackGroundColor','white','FontSize', 10,'Value', PAR.PLOT_NEG,...
    'callback',@negToggle_Callback);

        
uicontrol('style','text','units', 'normalized',...
    'position',[0.56 0.43 1.0 0.04 ],'FontSize', 11,...
...
CURSOR_TEXT = uicontrol('style','text','units', 'normalized',...
    'position',[0.56 0.39 1.0 0.04 ],'FontSize', 11,...
    'HorizontalAlignment','left','BackGroundColor', 'white', 'string','');

BA_TEXT = uicontrol('style','text','units', 'normalized',...
    'position',[0.56 0.36 1.0 0.04 ],'FontSize', 11,...
    'HorizontalAlignment','left','BackGroundColor', 'white', 'string','');

MAG_TEXT = uicontrol('style','text','units', 'normalized',...
    'position',[0.56 0.33 1.0 0.04 ],'FontSize', 11,...
    'HorizontalAlignment','left','BackGroundColor', 'white', 'string','');

SHOW_PEAK_DROPDOWN = uicontrol('style','popup','units','normalized',...
    'position',[0.64 0.43 0.2 0.04],'String','CTF Coords (cm)',...
    'Backgroundcolor','white','fontsize',10,'value',1,...
    'callback',@peakDropdown_Callback);

PLOT_PEAK = uicontrol('style', 'PushButton', 'units',...
    'normalized','position', [0.56 0.3 0.15 0.03],...
    'FontSize', 10,'string','Plot Virtual Sensor','ForegroundColor','blue',...
    'callback',@plot_peak_callback);

PLOT_PSF = uicontrol('style', 'PushButton', 'units',...
    'normalized','position', [0.56 0.25 0.15 0.03],...
    'FontSize', 10,'string','Plot CTF/PSF Images','ForegroundColor','blue',...
    'callback',@plot_psf_callback);

SHOW_PEAK_TOGGLE = uicontrol('style','checkbox','units', 'normalized',...
    'position',[0.56 0.21 0.1 0.03],'String','Find peaks',...
...
LIST_PEAK_TEXT = uicontrol('style', 'text','units','normalized','String','X     Y      Z     Mag.',...
    'horizontalAlignment','left','position', [0.57 0.18 0.35 0.03],...
    'FontSize',9,'BackgroundColor','white');

LIST_PEAK_SCROLL = uicontrol('style', 'listbox', 'units',...
    'normalized','position', [0.56 0.03 0.42 0.15],'max',1000,'Background',...
    'White', 'FontSize', 10,'string','','callback',@peak_scroll_callback);


GOTO_MNI = uicontrol('style', 'PushButton', 'units',...
    'normalized','position', [0.85 0.44 0.08 0.03],...
    'FontSize', 10,'string','Goto MNI','ForegroundColor','blue',...
    'callback',@goto_mni_callback);
...
% needs to be un-indented?
    function playMovie(FILE_LIST, PAR, format, movie_name)
        ...
        set(PLOT_PSF,'visible','off')
        set(GOTO_MNI,'visible','off')
...
%             M(k)=getframe(hMainFigure);    % this seems to be required to force draw update
    ...
    function peakDropdown_Callback(~,~)
        if ~PAR.IS_NORMALIZED 
            return;
        end        
        coordtype = get(SHOW_PEAK_DROPDOWN,'value');        
        if PAR.SHOW_PEAKS
            updateDisplay;
            drawCursors;
        end
    ...
    function peak_scroll_callback(src, ~)
        ...
        if isempty(temp_list)
            ...
            CURRENT_PEAK = temp_list(selected_rows(1),1:3);
        end
    ...
    function goto_mni_callback(~,~)
        
        % get current mni location
        peak = CURRENT_PEAK;

        % if displaying Tal put in MNI coordinates
        if coordtype == 2
            peak = round(bw_tal2mni(peak));
        end

        s1 = sprintf('%.2f', peak(1));
        s2 = sprintf('%.2f', peak(2));
        s3 = sprintf('%.2f', peak(3));
        input = inputdlg({'X (mm)'; 'Y (mm)'; 'Z (mm)'},'Select MNI Voxel',[1 50; 1 50; 1 50], {s1; s2; s3} );         
        if isempty(input)
            return;
        end
        
        mni =[ str2double(input{1}) str2double(input{2}) str2double(input{3}) ];
        
        % if displaying Tal put in Tal coordinates
        if coordtype == 2
            mni = round(bw_mni2tal(mni));
        end

        CURRENT_PEAK = mni;
        cursorPosition = CURRENT_PEAK;
        peakDropdown_Callback; % reset menu and display
    ...
    function plot_peak_callback(~, ~)
        ...
        % check for multiple peaks selected
 

        num_peaks = size(selected_rows,2);

        
        plotCount = 1;

        for count = 1:num_peaks
            
            temp_list = getPeakList(PAR);
        
            peak_voxel = temp_list(selected_rows(count),1:3);                        
                    
            if SUBJECT_NO == 0
                subjectsToPlot = NUM_SUBJECTS;
                          
                % Version 5.3 - ask by default with multiple subjects. 
                % - provide option to unwarp w or w/o search radius
                % now possible with non-normalized coordinates 
                    
                [useSR, searchRadius] = getTalSearchOption(peak_voxel, 1, params.vs_parameters.searchRadius);            
                if isempty(useSR)
                    return;
                else
            ...
            for k=1:subjectsToPlot  
                ...
                if PAR.IS_NORMALIZED
                    ...
                    if ~exist(tfile,'file')
                        s = sprintf('Could not find transform file %s. You may need to import Freesurfer or CIVET surfaces.', tfile);
                        errordlg(s);
                    end
                    transforms = load(tfile);
                    MNI_TO_MEG = transforms.MNI_to_MEG;
                    voxel = bw_mni2meg(MNI_TO_MEG,mni_voxel);
                else
                    voxel = peak_voxel;
                end         
   ...
   function plot_psf_callback(~, ~)
        ...
        r = questdlg('Plot Crosstalk  and Point-spread Function images for this peak location?','Plot PSF','Yes','No','No');
        if strcmp(r,'No')
            return;
        end
        ...
        if SUBJECT_NO == 0
            subjectsToPlot = NUM_SUBJECTS;
            % Version 5.3 - ask automatically only if group peak... 
            % provide option to unwarp w or w/o search radius
            % now possible with non-normalized coordinates 
            [useSR, searchRadius] = getTalSearchOption(peak_voxel, 1, params.vs_parameters.searchRadius);            
            if isempty(useSR)
        ...
        for k=1:subjectsToPlot  
            ...
            covDsName = char(imageset.covDsName(thisSubject));
            bw_make_psf(dsName, covDsName, params, voxel, PAR.IS_NORMALIZED);
    ...
    function plotSurfaceImage(src, ~)
        ...
        % check if displaying average
        if SUBJECT_NO == 0
           
            r = questdlg('Overlay all images or Average?','Overlay Images','All images','Average','Average');
            if strcmp(r,'All images')
                 overlay_list = strtrim(char(imageset.imageList));
            else
                overlay_list = char(imageset.averageList);
            end
    ...
    function drawImage
        ...
        % ** ver 4.3 - shifted overlay to SPM grid by 1 mm for better
        % alignment of cursor
        xstart = overlayGrid.bbox(1);
        ystart = overlayGrid.bbox(3);
        zstart = overlayGrid.bbox(5) + 1;
        xend = overlayGrid.bbox(2);
        yend = overlayGrid.bbox(4);
        zend = overlayGrid.bbox(6) + 1;
    ...
    function drawCursors   
        ...
        if PAR.IS_NORMALIZED
            ...
            elseif coordtype == 2
                label = getTalairachLabel(cursorPosition);
    ...
    function filename = loadImageData(FILE_LIST, IMAGE_NO, PLOT_NEG)
        ...
        if ~exist('filename')
            errordlg('Cannot find file. Current working directory has changed?');
        end
          ...
          for k=1:size(peaks,1)
            switch coordtype
              ...
              case 2
                ...
                coords = round(bw_mni2tal(original_coords));
                zvalue = peaks(k,4);               
                label = getTalairachLabel(original_coords);
...
function [useSR, searchRadius] = getTalSearchOption(peak_voxel, useSR, searchRadius)

    
    scrnsizes=get(0,'MonitorPosition');
    dh=figure('color','white','name','Plot Virtual Sensors...','numbertitle','off','menubar','none',...
    ...
    s = sprintf('Plotting Virtual Sensor for MNI coordinates:   X = %d  Y = %d  Z = %d', round(peak_voxel));

    uicontrol('Style','text','Units','Normalized','HorizontalAlignment','Left','fontweight','bold','Position',...
        [0.06 0.8 0.8 0.1],'String',s,'fontsize',12,'BackgroundColor','White');
           
    RADIO_EXACT=uicontrol('style','radiobutton','units','normalized','position',[0.05 0.6 0.6 0.2],'BackgroundColor','White',...
        'string','Use exact coordinates for all datasets','fontsize',12,'value', ~useSR, 'callback',@RADIO_EXACT_CALLBACK);
    
    RADIO_SEARCH=uicontrol('style','radiobutton','units','normalized','position',[0.05 0.4 0.6 0.2],'BackgroundColor','White',...
        'string','Use largest peak within a','fontsize',12,'value', useSR, 'callback',@RADIO_SEARCH_CALLBACK);
