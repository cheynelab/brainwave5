...
//				mexPrintf("excluding bad channel %s \n", badChannelNames[i]);
    ...
    mexPrintf("Creating new dataset: (numChannels:  %d, numSamples: %d)\n",  newParams.numChannels, newParams.numSamples);
    mexEvalString("drawnow");
	...
	for (int i=0; i<numLatencies; i++)
	{
		...
		for (int k=0; k<dsParams.numChannels; k++)
		{
            ...
            // ** 4.2 bug fix - was using variable type *int* here which resulted in wraparound of byteOffset for large datasets
            //
			long int epochOffset = (k * dsParams.numSamples) + startSample - preFilterPts;
            
			long int byteOffset = ( epochOffset * sizeof(int) ) + 8; // include ID string

//            sprintf(msg,"reading channel %s, epochOffset %ld, byteOffset %ld\n", dsParams.channel[k].name, epochOffset, byteOffset );
//            mexPrintf(msg);
//            mexEvalString("drawnow");
...
//        mexPrintf("Writing %d samples for epoch %d \n", newParams.numSamples*newParams.numChannels, i );
//        mexEvalString("drawnow");
