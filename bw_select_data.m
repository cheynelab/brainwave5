...
fh = figure('color','white','name','Brainwave - Channel Selection',...
    'numbertitle','off', 'Position', [scrnsizes(1,4)/2 scrnsizes(1,4)/2  900 900], 'closeRequestFcn', @cancel_button_callBack);
if ispc
    movegui(fh,'center');
end
[~,~,ext] = bw_fileparts(dsName);
...
plotHdl = plot3(position(:,1),position(:,2),position(:,3));
hDatatip = datacursormode(fh);
set(hDatatip,'enable','on','UpdateFcn',@UpdateCursors);
hDatatip.removeAllDataCursors;
    
hold on
set(plotHdl,'LineStyle','none','marker','o')
...
function [newText, pos] = UpdateCursors(src,evt)
    pos = get(evt,'Position');
    set(src,'MarkerSize',8)
    idx = find(pos(1) == position(:,1) & pos(2) == position(:,2)  & pos(3) == position(:,3));
    name = channelNames(idx,:);
    newText = sprintf('Channel %s (%.1f, %.1f, %.1f)', name, pos(1), pos(2), pos(3));
    
    includeNames = get(displaylistbox,'String');
    idx = find( contains(includeNames, name) == 1);
    if isempty(idx)
        excludeNames = get(hidelistbox,'String');
        idx = find( contains(excludeNames, name) == 1);
        set(hidelistbox,'value',idx);
    else
        set(displaylistbox,'value',idx);
    end
...
uicontrol('units','normalized','Position',[0.57 0.465 0.15 0.06],'String','Save List',...
...
%temp. text
uicontrol('style','text','units','normalized','position',[0.57 0.65 0.36 0.2],'string',...
...
uicontrol('style','text','units','normalized','position',[0.1 0.95 0.8 0.04],...
   ...
   function updateChannelLists
        ...
        % make sure we are not setting list beyond range.
