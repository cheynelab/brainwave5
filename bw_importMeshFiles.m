...
isFSaverage = 0; % ***** set to 1 for importing fsaverage surfaces only **** ...
meshFile = [];
...
% imports all CIVET meshes in chosen folder
 function importCIVETmeshes(mesh_dir, mri_dir)
        ...
        for h=1:size(surfaces,1)
            ...
            vertices(:,4) = [];
            mesh.meg_vertices = single(vertices);

            % get normal vectors...

            waitbar((h/num_surf), wbh, 'computing surface normals... ',k);
         
            nvoxels = size(vertices,1);
            voxels = vertices;
            ...
            % new for version 5.0 - drawing vertices are now MNI

            if ~isempty(normalized_vertices)
                mesh.normalized_vertices = single(normalized_vertices);  
                mesh.vertices = mesh.normalized_vertices;
            else
                mesh.normalized_vertices = [];  % not sure why this would happen?
                mesh.vertices = [];
           end
...
% imports all FSL meshes in chosen folder
 function importFSmeshes(mesh_dir, mri_dir)
        ...
        if isempty(sf)
            errordlg('This Freesurfer directory does not appear to contain any surfaces. Run recon-all with -all argument');
            return;
        end
        ...
        % check for head segmentation
        sf = dir(fullfile(mesh_dir, 'surf', 'lh.seghead'));
        if ~isempty(sf)
            surfaces{5,1} = 'scalp';

            sf = dir(fullfile(mesh_dir, 'surf', 'lh.seghead'));
            if size(sf,1) == 1
                surfaces{5,2} = fullfile(mesh_dir, 'surf', char(sf.name));
            end
        end
        ...
        for h=1:size(surfaces,1)
            ...
            ras_vertices = mesh.vertices;

            mesh.mri_vertices = single(ras_vertices);
            ...
            if h == 4
                ...
                t_vertices = ras_vertices;
            end

            % ****  fix in 5.0 - this scaling was not being included in the saved
            % transform RAS_to_MNI. Changed to apply scaling to mm to
            % RAS_to_MNI affine transformation so that 
            % conversions are correct if used outside of this routine! 
            ...
            %%%%
            % 3. compute vertices in MEG (head) coordinates using RAS_to_MEG
            ...
            % swap in pial vertices for MEG coords so that coordinate
            % lookup table returns correct values.
            if h == 4
                t_vertices = pial_vertices;
                t_vertices = [t_vertices, ones(size(t_vertices,1), 1) ];
                t_vertices = t_vertices * RAS_to_MEG;  % transform to head coordinates and scale to cm
            ...
            % 4. set mesh.vertices as the drawing vertices 
            % ** changed in version 5.0 to mni vertices
            if (h==4)
                t_vertices = ras_vertices;
                t_vertices = [t_vertices, ones(size(t_vertices,1), 1) ];
                t_vertices = t_vertices * RAS_to_MNI;  % transform to head coordinates and scale to cm
                t_vertices(:,4) = [];
                % shift inflated brain up and reduce size to match xlim
                if isFSaverage
                    t_vertices = t_vertices * 0.9;
                    t_vertices(:,3) = t_vertices(:,3) + 20;
                    t_vertices(:,2) = t_vertices(:,2) - 20;
                else
                    t_vertices = t_vertices * 0.7;
                    t_vertices(:,3) = t_vertices(:,3) + 30;
               end
                mesh.vertices = single(t_vertices);
            else
                mesh.vertices = single(mesh.normalized_vertices);
            end    
            ...
            %    ** changed in version 5.0 to MNI vertices in mm
            % 4. meg_vertices: 
            ...
            if mesh.numRightVertices > 0
                mesh.numVertices = mesh.numLeftVertices + mesh.numRightVertices;
                mesh.numFaces = mesh.numLeftFaces + mesh.numRightFaces;
                mesh.normals = single(normals);                             % normals in MEG coord!                      
            else                        
                mesh.numVertices = mesh.numLeftVertices;
                mesh.numFaces = mesh.numLeftFaces;
                mesh.normals = single(normals);                  
            end
